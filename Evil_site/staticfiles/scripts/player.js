...
var Update_Loading_Value, track_name, track_author, track_id, repeat_flag, shuffle_flag, volume, track_loaded, trackname;
var trackauthor, tracktime, Starting, search_value, MyMusic_element, AllMusic_element, MyMusic_switch, search_input;
...
track_author = localStorage.getItem("track_author");
...
image = localStorage.getItem("main_image");
if(image==null) image="";


repeat_flag = parseInt(localStorage.getItem("repeat_flag"));
if((repeat_flag==null) || isNaN(repeat_flag)) repeat_flag=0;
shuffle_flag = parseInt(localStorage.getItem("shuffle_flag"));
...
MyMusic_element = document.getElementById("MyMusic");
AllMusic_element = document.getElementById("AllMusic");
MyMusic_switch = localStorage.getItem("MyMusic_switch");
...
var play, pause, shuffle_element, repeat_element, for_repeat_element;
var add_to_music, MusicList_box, slider, track_list;

play = document.getElementById("play");
pause = document.getElementById("pause");
shuffle_element = document.getElementById("shuffle_flag");
repeat_element = document.getElementById("repeat_flag");
for_repeat_element = document.getElementById("for_repeat");
add_to_music = document.getElementById("add_to_music");
MusicList_box = document.getElementById("MusicList_box");
main_image = document.getElementById("main_img");

if(image) {
	main_image.src = "data:image/png;base64," + image;
...
function skip(side){
	...
	trackauthor.innerHTML=track_author;
	if(timed.image) {
		main_image.src = "data:image/png;base64," + timed.image;
		main_image.style.display = "block";
	} else{
		main_image.src = "";
		main_image.style.display = "none";
	...
	localStorage.setItem("track_author",track_author);
	localStorage.setItem("track_id",track_id);
	localStorage.setItem("main_image", timed.image);
	track_loaded=false;
	playpause();
}
...
function load_track(id){
	return $.ajax({
		...
		data: {"load": "true"},
		dataType: "json",
		error: function(response) {create_message(-1, "Error on download music",response);}
	});
}
...
function addtomusic(argument){
	let id=parseInt(argument.getAttribute("data"));
		...
		success: function(response){
			if(response.success=="true") {
				...
				if(argument==add_to_music) {
					const divs=MusicList_box.getElementsByTagName("div");
					for (const div of divs) {
						let track=parseInt(div.getAttribute("data"));
...
function UpdateRange(argument){
	if(myAudio.duration!==undefined) {
		...
		seconds=Math.floor(myAudio.currentTime);
		allseconds=Math.floor(myAudio.duration);
...
function StartTrack(argument,event,target){
	...
	let music_id, timed;
	music_id=parseInt(argument.getAttribute("data"));
	timed=undefined;
	for (const index in track_list) {
	...
	return $.ajax({
		...
		success: function(response){
			...
			for (const index in response) {
				...
				MusicList_box.appendChild(get_track_html(title=data["title"],author=data["author"],image=data["photo"],id=data["id"],time=data["time"],inmymusic=data["inmymusic"]));
...
function get_track_html(title, author, image, id, time, inmymusic) {
	let title_element, author_element, image_element, actions_element, add_element;
	title_element = document.createElement("div");
	title_element.setAttribute("class","title");
	title_element.innerHTML=title;
	
	author_element = document.createElement("div");
	author_element.setAttribute("class","author");
	author_element.innerHTML=author;
	
	if(image){
		image_element = document.createElement("img");
		image_element.setAttribute("src", "data:image/png;base64," + image);
	} else
		image_element = document.createElement("div");

	image_element.setAttribute("class", "image");
	image_element.setAttribute("alt", "");

	actions_element = document.createElement("div");
	actions_element.setAttribute("class","actions");

	add_element = document.createElementNS("http://www.w3.org/2000/svg","svg");
	...
	let load_element, info_element, time_element, track_element;
	
	load_element = document.createElementNS("http://www.w3.org/2000/svg","svg");
	...
	load_element.onclick = function(event) {
		create_message(-1, "Данный трек ещё не проверен на авторские права.");
	}

	info_element = document.createElementNS("http://www.w3.org/2000/svg","svg");
	info_element.setAttribute("class","add-icon");
	info_element.setAttribute("width","46");
	info_element.setAttribute("height","46");
	info_element.setAttribute("data",id);
	info_element.setAttribute("viewBox","0 0 46 46");
	info_element.setAttribute("fill","none");
	info_element.innerHTML='<line x1="10" y1="12" x2="36" y2="12" stroke="currentColor" stroke-width="4" stroke-linecap="round" />\
				<line x1="10" y1="22" x2="36" y2="22" stroke="currentColor" stroke-width="4" stroke-linecap="round" />\
				<line x1="10" y1="32" x2="36" y2="32" stroke="currentColor" stroke-width="4" stroke-linecap="round" />\
				<rect x="1" y="1" width="44" height="44" rx="21" stroke="currentColor" stroke-width="2" />';

	info_element.onclick = function(event) {
		window.location = LOAD_MUSIC_URL + "/" + id;
	}
	...
	actions_element.appendChild(info_element);
	actions_element.appendChild(time_element);
			

	track_element = document.createElement("div");
...
function set_point(percent,color){
	...
	let angle, d;
	angle = percent * 360;
	d = describeArc(50, 50, 48, 0, angle);
